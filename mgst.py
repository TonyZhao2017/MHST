...
class GDEncoder(nn.Module):
    def __init__(self, args):
        ...
        self.kff = nn.Linear(self.n_head * self.att_out, self.n_head * self.att_out)
        self.vff = nn.Linear(self.n_head * self.att_out, self.n_head * self.att_out)

        self.cff = nn.Conv2d(self.lstm_encoder_size, self.n_head * self.att_out, kernel_size=(3,1), padding=(1,0))
        ...
        self.second_glu = GLU(
            input_size=self.lstm_encoder_size,
            hidden_layer_size=self.lstm_encoder_size,
            dropout_rate=self.dropout)
    ...
    def forward(self, hist, nbrs, mask, va, nbrsva, lane, nbrslane, cls, nbrscls):
        ...
        # nbr car map to space
        soc_enc = t.zeros_like(mask).float()
        soc_enc = soc_enc.masked_scatter_(mask, nbrs_hidden_enc)
        # data preprocess end
        # egoCar: (128,16,64), batch,time,feature
        # nbrCar: (16,128,39,64), time,batch,space,feature
        ###
        ...
        soc_enc = soc_enc.permute(1, 3, 2, 0)
        c1 = self.cff(soc_enc)
        soc_enc = c1.permute(3, 0, 2, 1)
...
class Generator(nn.Module):
    def __init__(self, args):
        ...
        #  1          2       3
        # (16, 25, 6)
        self.mapping = t.nn.Parameter(t.Tensor(self.in_length, self.out_length, self.lstm_encoder_size + self.lat_length + self.lon_length))
    ...
    def forward(self, values, lat_enc, lon_enc):
        ...
        if self.train_flag:
            ...
            index = t.cat((maneuver_state, lat_man, lon_man), dim=-1).permute(-1, 0)
            ...
            for k in range(self.lon_length):
                for l in range(self.lat_length):
                    ...
                    lat_enc_tmp[:, l] = 1
                    lon_enc_tmp[:, k] = 1
                    index = t.cat((maneuver_state, lat_enc_tmp, lon_enc_tmp), dim=-1).permute(-1, 0)
                    mapping = F.softmax(t.matmul(self.mapping, index).permute(2, 1, 0), dim=-1)
                    dec = t.matmul(mapping, values).permute(1, 0, 2)
